...
export async function exportSlotsXLSX(eventId, eventName) {
  const [
    ...
    supabase.from('slots_score_entries')
      ...
      .eq('event_id', eventId).order('day_number').order('saved_at'),
    supabase.from('slots_commits').select('*').eq('event_id', eventId).order('day_number'),
    supabase.from('slots_spins')
      .select('*, slots_players(display_name)')
      .eq('event_id', eventId).order('spun_at', { ascending: false }).limit(5000),
    supabase.from('slots_prize_board')
      .select('*, slots_players(display_name), slots_store_items(label)')
      .eq('event_id', eventId).order('purchased_at'),
    ...
    supabase.from('slots_audit_log')
      .select('*, profiles(email)')
      .eq('event_id', eventId).order('created_at', { ascending: false }).limit(2000),
  ]);
  ...
  // ── Sheet 3: Commits (expanded per-player) ─────────────────────────────────
  ...
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(spinRows), 'Spin History');
  ...
  const prizeRows = (prizeBoard ?? []).map(p => ({
    ...
    'Fulfilled':    p.paid ? 'Yes' : 'No',
    'Fulfilled At': p.paid_at ? new Date(p.paid_at).toLocaleString() : '--',
  }));
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(prizeRows), 'Prize Board');

  // ── Sheet 6: Pay Table ─────────────────────────────────────────────────────
  const payoutRows = (payoutTable ?? []).map(r => ({
    'Outcome':    r.label,
    'Category':   r.category,
    'Symbols':    (r.symbols || []).join(' + '),
    'Payout (CPC)': r.payout_cpc,
    'Is Jackpot': r.is_jackpot ? 'Yes' : 'No',
  }));
  XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(payoutRows), 'Pay Table');
