...
function MatchupCard({ match, teams, maxRound, bracketType }) {
  ...
  return (
    <div className="card" style={{ padding: '0.75rem', borderColor: isChampion ? 'var(--accent-gold)' : isLosersChampion ? 'var(--accent-purple)' : 'var(--border)', boxShadow: isChampion ? 'var(--shadow-glow)' : 'none', minWidth: '220px' }}>
      {isChampion && <div style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--accent-gold)', marginBottom: '0.5rem', textAlign: 'center' }}>🏆 Champion</div>}
      {isLosersChampion && <div style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--accent-purple)', marginBottom: '0.5rem', textAlign: 'center' }}>🥉 3rd Place</div>}
      {[{ team: team1, score: match.team1_score, wins: match.series_wins_team1, id: match.team1_id },
        ...
        <div key={i}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.4rem 0.5rem', borderRadius: 4, background: match.winner_id === side.id ? 'rgba(46,196,182,0.1)' : 'transparent' }}>
            <span style={{ fontSize: '0.82rem', fontWeight: 600, color: match.winner_id === side.id ? 'var(--win)' : match.winner_id && match.winner_id !== side.id ? 'var(--text-muted)' : 'var(--text-primary)' }}>
              {side.team ? side.team.display_name : <span style={{ color: 'var(--text-muted)' }}>TBD</span>}
            </span>
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
              {side.wins > 0 && <span style={{ fontSize: '0.7rem', color: 'var(--win)', fontFamily: 'var(--font-mono)' }}>{side.wins}W</span>}
              {side.score !== null && side.score !== undefined && <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.88rem', fontWeight: 700 }}>{formatScore(side.score)}</span>}
            </div>
          </div>
        </div>
      ))}
      {!match.is_finalized && <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', textAlign: 'center', marginTop: '0.5rem' }}>Pending</div>}
    </div>
  )
}
            ...
            return (
              <div key={round} style={{ display: 'flex', flexDirection: 'column', minWidth: '240px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', justifyContent: 'space-around', flex: 1 }}>
                  {roundMatches.map(match => <MatchupCard key={match.id} match={match} teams={teams} maxRound={maxRound} bracketType={bracketType} />)}
                </div>
              </div>
            )
          })}
