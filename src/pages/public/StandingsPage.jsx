...
import { sortStandings, formatScore } from '../../lib/scoring'
...
export default function StandingsPage() {
  ...
  async function fetchAll() {
    ...
    setTeams(teamsData || [])
    const dayMap = {}
    leagueData?.forEach(row => { if (!dayMap[row.day_number]) dayMap[row.day_number] = row.league_average_score })
  ...
  const getDivisionStandings = (divisionId) => {
    ...
    return sortStandings(standings.filter(s => divTeamIds.includes(s.team_id)))
  }
  ...
  return (
    <div className="page-container">
      ...
      <div className="page-content">
            ...
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
              {dayNumbers.map(day => (
                <div key={day} className="stat-card" style={{ minWidth: '100px', flex: '0 0 auto' }}>
                  ...
                  <div className="stat-value" style={{ fontSize: '1.2rem' }}>{formatScore(leagueAvgByDay[day])}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {divisions.map(div => {
          ...
          return (
            <div key={div.id} style={{ marginBottom: '2.5rem' }}>
              <h2 style={{ fontSize: '1rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--accent-gold)', marginBottom: '1rem' }}>{div.name}</h2>
                  ...
                  <table>
                    <thead>
                      <tr>
                        ...
                        <th style={{ textAlign: 'center' }}>PTS</th><th style={{ textAlign: 'center' }}>W</th>
                        <th style={{ textAlign: 'center' }}>T</th><th style={{ textAlign: 'center' }}>L</th>
                        <th style={{ textAlign: 'center' }}>LA-W</th><th style={{ textAlign: 'center' }}>LA-T</th>
                        <th style={{ textAlign: 'center' }}>LA-L</th><th style={{ textAlign: 'center' }}>AVG</th>
                      </tr>
                    </thead>
                    <tbody>
                      {divStandings.length === 0
                        ...
                        : divStandings.map((s, i) => {
                            ...
                            return (
                              <tr key={s.team_id}>
                                <td><span className={`seed-badge ${i < 2 ? 'top' : ''}`}>{i + 1}</span></td>
        ...
        <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>PTS = Total points · W/T/L = Head-to-head · LA-W/T/L = vs. League Average · AVG = Average daily score</div>
      </div>
    </div>
  )
}
