...
export default function BoardGameEditPage() {
  ...
  const [activeTab, setActiveTab] = useState('board'); // 'board' | 'config' | 'players'

  const load = useCallback(async () => {
    const [evRes, cfgRes, sqRes, plRes] = await Promise.all([
  ...
  const handleSaveSquares = async () => {
    ...
    setMessage(null);
    try {
      // Delete all existing squares for this event, then re-insert
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      // Also update event name/dates
      const { error: evErr } = await supabase
        .from('events')
        .update({ name: event.name, start_date: event.start_date || null, end_date: event.end_date || null })
        .eq('id', eventId);
  ...
  const handleSavePlayer = async (player) => {
    ...
    if (error) {
      ...
      setMessage({ type: 'success', text: `${player.name} updated.` });
    }
  };
  ...
  const handleRemovePlayer = async (playerId, playerName) => {
    if (!confirm(`Remove ${playerName}? All their score entries will also be deleted.`)) return;
          ...
          {activeTab === 'board' && (
            <div>
              <p style={{ fontSize: 13, opacity: 0.6, marginBottom: 16 }}>
                Drag tiles to reposition. Click any tile to edit flavor text, description, or other properties. Click an empty square to add a new tile.
              </p>
              <BoardBuilder
                squares={squares}
                onChange={setSquares}
              ...
              <div style={{ marginTop: 16, display: 'flex', gap: 10 }}>
                ...
                <span style={{ fontSize: 12, opacity: 0.5, alignSelf: 'center' }}>
                  This replaces all existing tiles for this event.
                </span>
              </div>
            </div>
          )}
