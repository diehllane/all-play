...
export default function ManageScorersPage() {
  ...
  async function fetchAll() {
    ...
    setEvent(ev)
    const { data: assignments } = await supabase.from('user_event_assignments').select('*, profiles(*)').eq('event_id', id)
  ...
  async function createScorer(e) {
    ...
    setMessage(null)
    try {
      const { data, error } = await supabase.auth.signUp({ email: newEmail, password: newPassword, options: { data: { role: 'scorer' } } })
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        ...
        <p style={{ color: 'var(--text-secondary)' }}>Scorer accounts can enter scores for any team in this event.</p>
      </div>
        ...
        <div className="card" style={{ marginBottom: '1.5rem' }}>
          ...
          {scorers.length === 0 ? (
            ...
            <div key={scorer.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 0', borderBottom: '1px solid var(--border)' }}>
              ...
              <button className="btn btn-danger btn-sm" onClick={() => removeScorer(scorer.id)}>Remove</button>
            </div>
          ))}
        </div>
        <div className="card">
          <div className="card-title">Create Scorer Account</div>
          <p style={{ fontSize: '0.875rem', color: 'var(--text-secondary)', marginBottom: '1.25rem' }}>Creates a login that can enter scores but cannot create events.</p>
          ...
          <div className="alert alert-info" style={{ marginTop: '1rem' }}>
            <strong>Note:</strong> In production, scorer creation should use a Supabase Edge Function with service role key to avoid email confirmation.
          </div>
        </div>
      </div>
    </div>
